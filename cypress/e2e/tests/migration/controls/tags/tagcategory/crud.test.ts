...
describe(["@tier2", "@tier2_D"], "Tag tagCategory CRUD operations", () => {
  it("Tag Category CRUD", function () {
    const tagCategory = new TagCategory(data.getRandomWord(8), data.getColor());
    tagCategory.create();
    exists(tagCategory.name);

    const updatedTagType = data.getRandomWord(8);
    const updatedColor = data.getColor();
  ...
  it("Tag category CRUD with member (tags)", function () {
    ...
    for (const currentTag of tagList) {
      currentTag.delete();
      tagAmount -= 1;
