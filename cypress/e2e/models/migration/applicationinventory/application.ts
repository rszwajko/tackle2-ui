...
export class Application {
  ...
  public static open(forceReload = false): void {
    ...
    if (forceReload) {
      cy.visit(Application.fullUrl, { timeout: 35 * SEC }).then((_) => {
        cy.get("h1", { timeout: 10 * SEC }).should(
          "contain",
          applicationInventory
        );
    ...
    cy.url().then(($url) => {
      if ($url != Application.fullUrl) {
        ...
        cy.get("h1", { timeout: 60 * SEC }).should(
          "contain",
          applicationInventory
        );
  ...
  edit(
    ...
    if (cancel) {
      ...
      if (
        ...
        updatedValues.description != this.description
      ) {
      ...
      if (
        updatedValues.sourceRepo &&
        updatedValues.sourceRepo != this.repoType
      ) {
  ...
  tagAndCategoryDontExist(tags: string[][]): void {
    tags.forEach(function (tag) {
      ...
      cy.get(appDetailsView.tagCategory).should("not.contain", tag[0]);
    });
  }
  ...
  private validateAffectedValues(appIssue: AppIssue): void {
    performWithin(this.name, () => {
      ...
      if (this.business) {
        validateTextPresence(
          'td[data-label="Business service"]',
          this.business
        );
      }
  ...
  private validateAffectedFilesModal(appIssue: AppIssue): void {
    ...
    for (
      let affectedFile = 0;
      affectedFile < appIssue.affectedFiles;
      affectedFile++
    ) {
  ...
  addDependencies(
    northbound?: Array<string>,
    southbound?: Array<string>
  ): void {
