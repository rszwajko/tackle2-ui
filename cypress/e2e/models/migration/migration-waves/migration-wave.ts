...
export class MigrationWave {
  ...
  public fillStartDate(date: Date): void {
    ...
    const currentMonth = currentStartDate.toLocaleString("en-us", {
      month: "long",
    });
  ...
  public fillEndDate(date: Date) {
    ...
    cy.get(MigrationWaveView.endDateInput).type(
      MigrationWave.formatDateMMddYYYY(date),
      {
        delay: 0,
      }
    );
  ...
  private static selectDateFromDatePicker(date: Date, currentMonth: string) {
    cy.get(MigrationWaveView.yearInput).type(
      `{selectAll}${date.getFullYear()}`
    );
  ...
  private removeApplicationsFromModel(applicationsList) {
    this.applications = this.applications.filter(
      (application) =>
        !applicationsList.some((app) => app.name === application.name)
    );
  }
