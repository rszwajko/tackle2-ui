...
>(
  ...
  const persistence = {
    ...
    provider: usePersistanceProvider<TValue>(
      isPersistanceProviderOptions(options)
        ? options
        : {
            serialize: () => {},
            deserialize: () => defaultValue,
...
const usePersistanceProvider = <TValue>({
  serialize,
  deserialize,
  defaultValue,
...
export const isPersistanceProviderOptions = <
  TValue,
  TPersistenceKeyPrefix extends string,
  TURLParamKey extends string,
>(
